...
Follow interactive menu and enter information about your study. This will create record for project id which will be rooting bunch of samples 

# Stage 1: Prepare biosamples entries [sample_xmler.pl]
Biosample records contains information about your sample specifics and will link this metadata to the project id sequence file. Following stepes will be required to run sample_xmler.pl  
...
•	collection_date
Here is example for two samples metadata_example.xlsx
...
In stage 1 you created database entries for biosample ID. Now we need create manifest files for assemblies using generated biosample ID with MANIFESTATOR.PL. To upload assembly you will require require 2 files
...
This script will take manifest_meta_table.txt table ind split it to individual manifests. Named by ID in the first column of metadata. The example of produced manifest file (10001.manifest): 
